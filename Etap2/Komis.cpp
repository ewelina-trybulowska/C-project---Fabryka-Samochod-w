...
void Komis::comission_buy(Owner &owner, int index){ //KOMIS KUPUJE AUTO
    if( index < owner.size_Vehicles_sold() && index >= 0){

        Pojazd* vehicle_buy = owner.get_vehicle_sold(index);

       vehicle_buy->changeOwner("komis");

       //przebieg wybranych marek dzielony przez 2: wybrałam audi i bmw
        if(strcmp(vehicle_buy->get_brand() ,"BMW")==0 || strcmp(vehicle_buy->get_brand(), "Audi")==0 ){
          int pti = vehicle_buy->get_mileage();
          int new_milage = pti / 2;
            vehicle_buy->changeMileage(new_milage);
        }

        motor_vehicles.push_back(dynamic_cast<PojazdSilnikowy*>(vehicle_buy));//rzutuje dynamic_cast, poniewaz
        //rzutuje w dol hierarchii dziedziczenia, rzutuje z obiektu klasy bazowej pojazd na pochodna pojazd silnikowy

        owner.remove_from_Vsold(index);
        ...
        std::cout<<" nie ma takiego pojazdu do sprzedania"<<std::endl;
    }
}

void Komis:: comission_sell(Owner &owner, int index, std::string kupujacy){ //KOMIS SPRZEDAJE
    if(index < motor_vehicles.size() && index >= 0){

        PojazdSilnikowy* vehicle = motor_vehicles.at(index);
        //vehicle->owner=std::move(kupujacy); //std move zeby nie kopiowac jeszcze raz i nie alokowac na to pamieci,
        vehicle->changeOwner(std::move(kupujacy));
        owner.add_to_Vsold(vehicle);//tutaj nie rzutuje , konwersja w gore wykonuje sie automatycznie
        //bo kazdy pojazd silnikowy jest pojazdem

        std::cout<<"Udalo sie!!! wlasnie kupiles pojazd z komisu!!"<<std::endl;
